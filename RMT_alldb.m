...
% 定义图像保存的文件夹路径
folder_path_img = 'D:\\Radar_Jamming_Signal_Dataset\\Trainning_data\\dataset_img\\All_dB\\RMT';
...
% 定义序列保存的文件夹路径
folder_path_seq = 'D:\\Radar_Jamming_Signal_Dataset\\Trainning_data\\dataset_seq\\All_dB\\RMT';
...
for SNR = -20:2:10
    for a = 1:data_num
        %%  目标回波（加入随机延时），并引入假目标，距离维密集假目标干扰 RMT
        ...
        range=200+round(rand(1,1)*1800);    %假目标滞后10~100us，即200-2000个点
        range_tar = 1 + round(rand(1, 1) * (samp_num - Nfast - 1-range-2000)); % 随机偏移量，表示目标信号在噪声信号中的起始位置，并确保range_tar不超过允许的范围 
        
    
        k=2+round(rand(1,1)*3); %改为随机产生2-5（个假目标）
        ...
        sp(1 + range_tar : Nfast + range_tar) = sp(1 + range_tar : Nfast + range_tar) + As * lfm(1:Nfast); % 噪声+目标回波，range_tar相当于就是随机时延偏移
         ...
         for i=0:k-1 %距离窗内添加多个假目标干扰
            delay_time_num=100+round(rand(1,1)*400); %假目标之间随机延迟5-25us
            sp(1 + range_tar +range+i*delay_time_num: Nfast + range_tar+range+i*delay_time_num) = sp(1 + range_tar +range+i*delay_time_num: Nfast + range_tar+range+i*delay_time_num) + Aj * lfm(1:Nfast); 
         end
