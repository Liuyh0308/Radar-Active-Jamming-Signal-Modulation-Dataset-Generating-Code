...
% 定义图像保存的文件夹路径
folder_path_img = 'D:\\Radar_Jamming_Signal_Dataset\\Trainning_data\\dataset_img\\All_dB\\AM';
...
% 定义序列保存的文件夹路径
folder_path_seq = 'D:\\Radar_Jamming_Signal_Dataset\\Trainning_data\\dataset_seq\\All_dB\\AM';
...
for SNR = -20:2:10
    for a = 1:data_num
        ...
        sp(1 + range_tar : Nfast + range_tar) = sp(1 + range_tar : Nfast + range_tar) + As * lfm(1:Nfast); % 噪声+目标回波
        echo = sp;  % 未加入噪声的回波信号
        ...
        sp1_fft(range:round(samp_num*((1-Bj/(fs/10e5)/2)))) = 0;  % 将变换后的信号中不属于瞄准干扰带宽内的低频部分置零
